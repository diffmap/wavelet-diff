...
class CovariantVPSDE(VPSDE):
    ...
    Args:
        ...
        num_levels: Number of wavelet bands in the correlation matrix
    """
    def __init__(self, bmin=0.1, bmax=20.0, level_correlations=None, num_features=4, num_levels=4):
        ...
        level_correlations = torch.as_tensor(level_correlations, dtype=torch.float32)
        expected_shape = (num_levels, num_levels)
        if tuple(level_correlations.shape) != expected_shape:
            raise ValueError(
                f"level_correlations must have shape {expected_shape}, got {tuple(level_correlations.shape)}"
            )
        self.register_buffer('level_correlations', level_correlations)
        self.register_buffer('cholesky_L', torch.linalg.cholesky(self.level_correlations))
    ...
    def correlated_noise(self, t):
        ...
        """
        base_std = super().g(t)  # [B, pred_len, 1, 1]
        B, T = base_std.shape[:2]
        ...
        correlated = torch.einsum("ij,bjf->bif", self.cholesky_L, noise_flat)
        # Reshape back.
        correlated = correlated.view(B, T, L, self.num_features)
        
        # Scale by base standard deviation
        return base_std * correlated  # [B, T, L, D]
